...
// --- Loss Functions via Linear Predictor (Exact Original Math) ---
...
// --- Penalty Function (Exact Original Math) ---
...
// --- Objective Function (Exact Original Math) ---
...
// --- Vectorized Block Linear Predictor (100% Mathematically Equivalent) ---
...
inline arma::vec compute_block_eta(
  ...
  // Mathematically identical to sum_jj X_l.col(jj) % (beta_l(jj) + Z * Theta_l.col(jj)),
  // but computes Z * Theta_l once via BLAS matrix multiplication.
  arma::vec eta_l = X_l * beta_l;
  arma::mat ZTheta_l = Z * Theta_l;
...
Rcpp::List fast_sgpl_fit_cpp2(
  ...
  // Allocating these once outside the loops prevents dynamic allocation overhead inside backtracking.
  arma::vec beta_curr(p);
  arma::mat Theta_curr(K, p);
  arma::vec beta_cand(p);
  arma::mat Theta_cand(K, p);
  ...
  for (int iter_out = 0; iter_out < max_iter_out; ++iter_out) {
    ...
    for (unsigned int l = 0; l < L; ++l) {
      ...
      // KKT Screening (Gaussian) - Vectorized matrix-matrix operation
      ...
      for (int iter_in = 0; iter_in < max_iter_in; ++iter_in) {
        ...
        // Populate base current state into pre-allocated workspace buffers
        beta_curr = beta;
        Theta_curr = Theta;
        beta_curr.elem(idx_l) = beta_l_tilde;
        Theta_curr.cols(idx_l) = Theta_l_tilde;

        // Evaluate baseline full objective function (Exact original logic)
        ...
        for (int bt = 0; bt < bt_max; ++bt) {
          ...
          // Populate candidate into pre-allocated workspace buffers (No new dynamic memory created)
          beta_cand = beta;
          Theta_cand = Theta;
          ...
          // Evaluate candidate objective using exact original function
